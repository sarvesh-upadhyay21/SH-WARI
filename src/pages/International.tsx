...
export default function International() {
  const { t } = useTranslation();
  const [estimate, setEstimate] = useState<{ low: number; high: number; name: string } | null>(
    null,
  );
  const form = useForm<EstimateData>({
    resolver: zodResolver(estimateSchema),
    defaultValues: { procedure: "", country: "" },
  });
          ...
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Button
                ...
                data-testid="btn-intl-teleconsult"
              >
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            <Badge className="bg-primary/10 text-primary border-0 mb-3">
              {t("international.services.badge")}
            </Badge>
        ...
        <div className="max-w-3xl mx-auto px-4">
          <div className="text-center mb-10">
            <Badge className="bg-accent/10 text-accent border-0 mb-3">
              {t("international.estimator.badge")}
            </Badge>
            <h2 className="text-4xl font-bold font-serif text-foreground">
              {t("international.estimator.title")}
            </h2>
            ...
            <CardContent className="p-8">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleEstimate)} className="space-y-5">
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <FormControl>
                            <SelectTrigger data-testid="select-estimate-procedure">
                              <SelectValue
                                placeholder={t("international.estimator.procedurePlaceholder")}
                              />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <FormControl>
                            <SelectTrigger data-testid="select-estimate-country">
                              <SelectValue
                                placeholder={t("international.estimator.countryPlaceholder")}
                              />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  >
                    ...
                    {t("international.estimator.getEstimate")}
                  </Button>
                </form>
              </Form>
              ...
              {estimate && (
                ...
                >
                  ...
                  <p className="text-3xl font-bold text-primary font-serif mb-1">
                    ${estimate.low.toLocaleString()} – ${estimate.high.toLocaleString()}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {t("international.estimator.indicativeRange")}
                  </p>
                  <Link href="/contact">
                    <Button
                      className="mt-4 bg-primary text-white w-full"
                      data-testid="btn-estimate-enquire"
                    >
                      {t("international.estimator.requestQuote")}
                    </Button>
                  </Link>
                </div>
              )}
