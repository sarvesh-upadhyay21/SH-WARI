          ...
          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-10">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {dept.treatments.map((t_) => (
                    <div
                      ...
                      data-testid={`treatment-${t_.toLowerCase().replace(/\s+/g, "-")}`}
                    >
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {dept.procedures.map((p) => (
                    <div
                      ...
                      data-testid={`procedure-${p.toLowerCase().replace(/\s+/g, "-")}`}
                    >
              ...
              {dept.faqs && dept.faqs.length > 0 && (
                <div>
                  <h2 className="text-2xl font-bold font-serif text-foreground mb-5">
                    {t("departments.faq")}
                  </h2>
                  <Accordion type="single" collapsible className="space-y-3">
                    {dept.faqs.map((faq, i) => (
                      <AccordionItem
                        key={i}
                        value={`faq-${i}`}
                        className="border border-border rounded-xl px-5 bg-muted/20"
                        data-testid={`dept-faq-${i}`}
                      >
              ...
              {deptDoctors.length > 0 && (
                <Card className="border border-border">
                  <CardContent className="p-6">
                    <h3 className="text-lg font-bold font-serif text-foreground mb-5">
                      {t("departments.specialists")}
                    </h3>
                      ...
                      {deptDoctors.map((doc) => (
                        <Link key={doc.id} href={`/doctors/${doc.id}`}>
                          <div
                            ...
                            data-testid={`dept-doctor-${doc.id}`}
                          >
